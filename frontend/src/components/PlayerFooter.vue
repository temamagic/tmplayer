<template>
  <footer class="footer" style="display:flex; gap:12px; align-items:center;">
    <img v-if="song" :src="song.cover || '/cover.jpg'" class="w-14 h-14 rounded object-cover" />

    <div style="flex:1; min-width:0;">
      <div style="display:flex; justify-content:space-between; align-items:center;">
        <div style="min-width:0">
          <div style="font-weight:600" class="truncate">{{ song?.title || 'No track' }}</div>
    ...
    <div style="display:flex; align-items:center; gap:8px;">
      ...
      <button class="btn" @click="$emit('togglePlay')">{{ isPlaying ? '⏸' : '▶️' }}</button>
